...
const TicketPage = () => {
    ...
    const getResult = async () => {
        ...
        try {
            const path = ConnectionInfo.url+'/stops/';
    ...
    <div className="container">
        ...
        <ul className="responsive-table">
            ...
            {BoardingStops && (
                <select className="col col-3" value={BoardingStop} onChange={e=>setBoardingStop(e.target.value)}><option disabled selected value></option>
                {BoardingStops.map((result) => (<option key = {result.id} value = {result.id}>{result.name}</option>))}
                </select>
            )}

            {DestinationStops && (
                <select className="col col-4" value={DestinationStop} onChange={e=>setDestinationStop(e.target.value)}><option disabled selected value></option>
